...
export function DisputeTable({
  ...
  const rowActions = useMemo(() => (dispute: Dispute) => 
    getDisputeRowActions(dispute, { onSubmitEvidence }), 
    [onSubmitEvidence]
  )

  const hoverActions = useMemo(() => (dispute: Dispute) => 
    getDisputeHoverActions(dispute, { onSubmitEvidence }), 
    [onSubmitEvidence]
  )
